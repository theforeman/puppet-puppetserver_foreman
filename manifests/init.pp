...
# @param manage_fact_watcher
#   If the fact_watcher service should be managed via puppet
...
class puppetserver_foreman (
  ...
  Boolean $manage_fact_watcher = pick($facts['is_pe'], false),
  Boolean $fact_watcher_service = pick($facts['is_pe'], false),
  Boolean $manage_fact_watcher_dependencies = pick($facts['is_pe'], false),
